...
function translatorTool() {
    /* Ex.  I sail across the water  
            The water was cold to the touch
            This surprised me: water is usually cool not cold
            I imagine the first time I make it to the Mountains of Zanzibar I will collapse because of the cold
            ...
            I [noun.cognition] the first time I make it to [noun.location] I will [noun.event] because of the [s2]
    */
    var args = { param:{}, found_param:found_param(), check_params:check_params(), seeking:seeking(), get_stack:get_stack(), stack:{}, index:0, word:0, attrib:0 }
    args.check_params(g_token.noun.act)
    args.check_params(g_token.noun.object)
    args.check_params(g_token.noun.state)
    args.check_params(g_token.noun.cognition)
    args.check_params(g_token.noun.location)
    args.check_params(g_token.noun.event)
    if(args.seeking()){    
        for(var i in mysql_wn_data_synsets_attributes){
            if(!g_used[i]){
                args.word = i
                args.attrib = mysql_wn_data_synsets_attributes[i][0][0]
                if(
                    args.found_param(g_token.noun.act) || 
                    args.found_param(g_token.noun.object) || 
                    args.found_param(g_token.noun.state) ||
                    args.found_param(g_token.noun.cognition) ||
                    args.found_param(g_token.noun.location) ||
                    args.found_param(g_token.noun.event)
                ){
                    g_used[i] = 1                
                }
                if(!args.seeking()){
    ...
    srcTranslated.value = Sentence(args)
}
// translatorTool ()
